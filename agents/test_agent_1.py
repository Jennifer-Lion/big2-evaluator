...
def checkHoldback(hand, field, enemy1, enemy2, enemy3, turn, classA, classB, classC, classD, selected_card):
    # Holdback Single
    if len(field) == 1:
        if len(hand) <= 2:
            return False
...
def advanceStrategy(hand, field, moveLists, classA, classB, classC, classD, enemy1, enemy2, enemy3, control, moveLen):
...
def moveSelection(hand, field, turn, classA, classB, classC, classD, enemy1, enemy2, enemy3, control, cardClass, moveLists):
    ...
    if moveLen <= 3 and len(hand) > 4:
        return advanceStrategy(hand, field, moveLists, classA, classB, classC, classD, enemy1, enemy2, enemy3, control, moveLen)
    ...
    if not control:
        ...
        if classD:
            for selected_card in classD:
                if not checkHoldback(hand, field, enemy1, enemy2, enemy3, turn, tempClassA, tempClassB, tempClassC, tempClassD, selected_card):
                    return selected_card
        if classC:
...
def action(hand, field, control, turn, field_history, enemy1, enemy2, enemy3):
    ...
    if enemy1 == 1: # action(hand, field, control, turn, field_history, enemy1, enemy2, enemy3)
        if control: 
            if len(moveLists[-1]) == 1: # if only can play singles, play the highest
                return moveLists[-1]
            else: # if not, just play the smallest in the non-singles category
                moveLists = [combo for combo in moveLists if len(combo) > 1]
                return moveLists[0]
        elif len(field) == 1: # singles are played
            return moveLists[-1]
        else: 
            return moveLists[0]
    ...
    return moveSelection(hand, field, turn, classA, classB, classC, classD, enemy1, enemy2, enemy3, control, cardClass, moveLists)
